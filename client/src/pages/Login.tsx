...
// 1. FIX: Changed headless to the normal package so it actually pops up on screen!
...
export default function Login({ state }: { state: string }) {
    ...
    // Sync state if it changes externally (e.g. clicking Nav links between Login/Register)
    useEffect(() => {
        setIsLoginState(state === "login");
    }, [state]);

    // 2. FIX: Changed React.SubmitEvent to React.FormEvent (the proper React type)
    const handleSubmit = async (e: React.FormEvent) => { 
        ...
        if (result.success) {
            ...
            // This will now successfully render the server's rejection message on screen
        ...
        <div className="min-h-screen flex items-center justify-center px-4">
            <div className="w-full max-w-md">
                ...
                <div className="bg-card border border-border rounded-2xl p-8">
                    <form onSubmit={handleSubmit} className="space-y-5">
                        <div className="text-center py-5">
                            <h1 className="text-2xl text-foreground">{isLoginState ? "Welcome back" : "Get Started"}</h1>
                        ...
                        <label>
                            ...
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    required
                                    value={email}
                                    onChange={(e) => setEmail(e.target.value)} // Fixed variable typo here as well
                ...
                <p className="text-center text-sm text-muted-foreground mt-6">
                    ...
                    <button type="button" onClick={() => setIsLoginState((prev) => !prev)} className="text-primary hover:underline font-medium pl-1">
